...
export const N_FREQ_FROM_ERB = (() => {
  let s = 0;
  for (const w of ERB_WIDTHS) s += w;
  return s;
})();
 ...
 * @param power [nFrames, N_FREQ_FROM_ERB] row-major
 * @returns [nFrames, N_ERB] row-major
 */
export function projectPower(power: Float32Array, nFrames: number): Float32Array {
