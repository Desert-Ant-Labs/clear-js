...
export interface STFTResult {
  /** [nFrames, nFreq] row-major real part. */
  real: Float32Array;
  /** [nFrames, nFreq] row-major imaginary part. */
  imag: Float32Array;
  nFrames: number;
}
...
export class STFT {
  ...
  private readonly wnorm: number;
  private readonly fft: BluesteinFFT;
  private readonly _rIn: Float64Array;
  private readonly _iIn: Float64Array;
  private readonly _rOut: Float64Array;
  private readonly _iOut: Float64Array;
  ...
  forward(audio: Float32Array): STFTResult {
    ...
    const rIn = this._rIn;
    const iIn = this._iIn;
    const rOut = this._rOut;
    const iOut = this._iOut;
  ...
  /** Inverse STFT. `real`/`imag` are [nFrames, nFreq]; prePad is trimmed off the front. */
  inverse(real: Float32Array, imag: Float32Array, nFrames: number): Float32Array {
